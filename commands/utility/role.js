...
module.exports = {
    ...
    async run (client, message, args) {
        let errEmbed = ccEmbed('error', 'Role Error!')
        ...
        switch (args[0]) {
            case '-create':
                ...
                message.guild.roles.create({
                    ...
                    const embed = ccEmbed('success', 'Role Created!', `Role <@&${role.id}> has been successfully created!`)
            ...
            case '-delete':
                ...
                try {
                    role.delete().then(role => {
                        const embed = ccEmbed('success', 'Role Deleted!',`Role \`${role.name}\` has been successfully deleted!`)
            ...
            case '-give':
                ...
                try {
                    memberToGiveTo.roles.add(roleToGive.id).then(member => {
                        const embed = ccEmbed('success', 'Role Successfully Given!', `Role <@&${roleToGive.id}> has been successfully given to <@${member.id}>`)
            ...
            case '-take':
                ...
                try {
                    memberToTakeFrom.roles.remove(roleToTake.id).then(member => {
                        const embed = ccEmbed('success', 'Role Successfully Taken!', `Role <@&${roleToTake.id}> has been successfully taken from <@${member.id}>`)
