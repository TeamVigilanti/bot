...
module.exports = {
    ...
    async run (client, message, args) {
        ...
        const errEmbed = ccEmbed('error', 'User Not Found!', 'Please specify a valid user!')
        ...
        switch (args[0]) {
            case '-add':
                ...
                if (await isBlacklisted(user.id)) {
                    let embed = ccEmbed('error', 'User Already Blacklisted!', 'This user is already blacklisted!')
                ...
                const baseEmbed = ccEmbed('success', 'User Successfully Blacklisted!',`User <@${user.id}> has successfully been stinky'fied!`)

                let iEmbed = baseEmbed.addField('User DM Status', 'Pending...')

                const msg = await message.channel.send({ embeds: [iEmbed] })

                user.send({ embeds: [ccEmbed('error', 'Oops!', 'You have been blacklisted from CrowdControl by the devs! Contact devs for the reason!')] })
                    .then(() => {
            ...
            case '-remove':
                ...
                if (!(await isBlacklisted(user.id))) {
                    let embed = ccEmbed('error', 'User Not Blacklisted!', 'This user is not blacklisted!')
                ...
                const wlBaseEmbed = ccEmbed('success', 'User Successfully Whitelisted!',`User <@${user.id}> has successfully been unstinky'fied!`)

                let wlIEmbed = wlBaseEmbed.addField('User DM Status', 'Pending...')

                const wlMsg = await message.channel.send({ embeds: [wlIEmbed] })

                user.send({ embeds: [ccEmbed('success', 'Yay!', 'You have been whitelisted from CrowdControl by the devs!')] })
                    .then(() => {
                ...
                break
            default:
                return message.channel.send({ embeds: [ccEmbed('error', 'Invalid Flag!' ,'Please use a valid flag! (`-add`, `remove`)')] })
        }
    }
}