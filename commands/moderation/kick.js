...
module.exports = {
    ...
    async run (client, message, args) {
        ...
        const kickEmbed = ccEmbed('success', 'User Kicked!')
            .addField('Kicked User', `target.user.tag (${target.id})`)
            .addField('Reason', reason)
        
        const kickUserEmbed = ccEmbed('error', 'Kicked!', `You have been kicked from ${message.guild.name}!`)
            .addField('Reason', reason)
            .setColor(0xFFFF00)
        ...
        target.kick({ reason })
            ...
            .catch(e => {
                return message.channel.send({ embeds: [ccEmbed('error', 'Unable to kick user!', 'Might be because of member having higher permissions!')] })
            })
        }
}